...
static void forward_token(const Model& m,Work& w,int token,int pos,int max_seq){
    ...
    int B=256;
    embedding_rmsnorm_kernel<<<1,B,B*sizeof(float)>>>(token,m.emb,m.layers[0].ln1,w.x,w.n,HIDDEN,m.rms_norm_eps);
    for(int i=0;i<N_LAYERS;i++){
        ...
        if(i+1<N_LAYERS){
            ...
            add_rmsnorm_kernel<<<1,B,B*sizeof(float)>>>(w.x,w.mo,m.norm,w.n,HIDDEN,m.rms_norm_eps);
        }
    }
